...
public static int contador_rojos;
public static int contador_azules;
    ...
    public InterfazGrafica() {
        ...
        hilo_color cambiar = new hilo_color(rojo,azul);
        Thread decolorar = new Thread(cambiar);
        decolorar.start();
        
        rojo.addActionListener((ActionEvent ae) -> {
            if (rojo.getBackground() == Color.blue) {
                contador_azules++;
                 text2.setText(String.valueOf(contador_azules));
                }
             if(rojo.getBackground() == Color.red){
                contador_rojos++;
                 text1.setText(String.valueOf(contador_rojos));
        }
         });
        
       azul.addActionListener((ActionEvent ae) -> {
            if(azul.getBackground() == Color.blue){
                 contador_azules++;
                 text2.setText(String.valueOf(contador_azules));
                }
             if(azul.getBackground() == Color.red){
                contador_rojos++;
                 text1.setText(String.valueOf(contador_rojos));
        }
         });
